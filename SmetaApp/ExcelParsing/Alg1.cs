                ...
                while (m.Count == 0 && cs <= ce)
                {
                    ...
                    for (int a = rs; a <= re; a++)
                    {
                        ...
                        Action<int> parseTable = (i) =>
                        {
                            ...
                            {
                                var codeCells = new IXLCell[2] { ws.Cell(i, c), ws.Cell(i + 1, c) };
                                if (check(codeCells[0]) || check(codeCells[1]))
                                    break;

                                string code = Regex.Match(codeCells[0].GetValue<string>(), @"\S+").Value + Regex.Match(codeCells[1].GetValue<string>(), @"\S+").Value;
